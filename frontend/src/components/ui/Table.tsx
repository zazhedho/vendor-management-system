...
export function Table<T>({
    ...
    if (data.length === 0) {
        return (
            <div className="w-full rounded-xl border-2 border-dashed border-secondary-200 bg-secondary-50/50 backdrop-blur-sm shadow-sm overflow-hidden">
        ...
        <div className="overflow-hidden rounded-xl border border-secondary-200 shadow-soft bg-white">
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="bg-gradient-to-r from-secondary-50 to-secondary-100/50 border-b border-secondary-200 sticky top-0 z-10 backdrop-blur-sm">
                        <tr>
                            {columns.map((col, index) => (
                                <th
                                    key={index}
                                    className={`px-6 py-4 text-left text-xs font-bold text-secondary-700 uppercase tracking-wider ${col.className || ''}`}
                                >
                                    {col.header}
                                </th>
                            ))}
                        ...
                        {data.map((item, rowIndex) => (
                            ...
                            >
                                {columns.map((col, colIndex) => (
                                    <td
                                        key={colIndex}
                                        className={`px-6 py-4 text-secondary-700 group-hover:text-secondary-900 transition-colors ${col.className || ''}`}
                                    >
